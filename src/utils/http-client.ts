...
export class HttpClient {
  ...
  constructor(
    apiKey: string,
    baseUrl: string = 'https://api.knowledgesdk.com',
    maxRetries: number = 3,
  ...
  async request<T = any>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    try {
      ...
      if (this.debugMode) {
        ...
        if (data) console.log('[KNOWLEDGESDK] Body:', JSON.stringify(data, null, 2));
      }
      ...
      if (response.status === 429) {
        if (retryCount < maxRetries) {
          ...
          if (this.debugMode) {
            console.log(
              `[KNOWLEDGESDK] Rate limited, retrying in ${nextDelay}ms (attempt ${retryCount + 1}/${maxRetries})`
            );
          }
      ...
      if (response.status >= 500) {
        if (retryCount < maxRetries) {
          ...
          if (this.debugMode) {
            console.log(
              `[KNOWLEDGESDK] Server error, retrying in ${nextDelay}ms (attempt ${
                retryCount + 1
              }/${maxRetries})`
            );
          }
